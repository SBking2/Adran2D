...
//顶点数据，额外的是骨骼ID和该顶点跟随骨头的权重，注意这里设定一个顶点最多能跟四个骨头有权重
struct Vertex {
	glm::vec3 position;
	glm::vec3 normal;
	glm::vec2 uv;
...
//这是一个骨头的数据,有他的ID，他的名字，他的Offset(是对于父结点的相对变换吗？), 可以包含他的所有子节点
struct Bone {
...
//动画，有总时长？每秒多少帧？根据骨骼名字找骨骼Transform
struct Animation {
	float duration = 0.0f;
...
void loadModel(const aiScene* scene, aiMesh* mesh, std::vector<Vertex>& verticesOutput, std::vector<uint>& indicesOutput, Bone& skeletonOutput, uint& nBoneCount) {
	...
	//这里加载了model的顶点法线UDUV，但是boneID和weight只给了初始值
